...
@Controller
@RequestMapping(value={"/nopayment"})
public class NoPaymentController {
	private final Log logger = LogFactory.getLog(getClass());
	
	@RequestMapping(method=RequestMethod.GET)
	public void approveNoCostNoPaymentCR(HttpServletRequest request, HttpServletResponse response, @RequestParam("crsID") String crsID, ModelMap model) {
		
		User user = (User) request.getSession().getAttribute("user");
		
    	logger.debug("NoPaymentController paymentPendingNotification");
		logger.debug("NoPaymentController CrsID: "+ crsID);
		
		String pl_msg = "Success";
		
		String message = forceSabaAuthentication(user.getUsername(), user.getPassword());
		if(!message.equals("")){
			logger.debug("NoPaymentController Error forcing SABA authentication");
			pl_msg = "Error forcing SABA authentication";
		}
		
		// Process purchase order
		pl_msg = getSabaHelper().approveNoCostNoPaymentCR(crsID);
	...
	private String forceSabaAuthentication(String username, String password){
		...
		/* Debugging */
		logger.debug("NoPaymentController SABA isLogin is " + getSabaHelper().islogin());
		logger.debug("NoPaymentController forcing SABA authentication");
