...
<head>
	...
	<script type="text/javascript" charset="utf-8">
		...
		function showSurvey(){
			window.setTimeout("displaySurvey()",3000);
		}
		...
		$(document).ready(function() {
			...
			showSurvey();
	  		...
	  		<core:if test="${courseRecord.feeOption=='Fee'}" >
			$('#printFullCertificates').click(function() {
				var certificatesURL = window.location.protocol.toString() + "//" + window.location.host.toString() + "/Saba/Web/Main/goto/FullCertificate?offeringId=${courseRecord.sheetNumberID}";
			  		
				window.open(certificatesURL,'_blank');
			});
			$('#printWalletCertificates').click(function() {
				var certificatesURL = window.location.protocol.toString() + "//" + window.location.host.toString() + "/Saba/Web/Main/goto/WalletCertificate?offeringId=${courseRecord.sheetNumberID}";
	...
	<div class="content student-page">
		...
		<section class="primary-content" role="main">
			...
			<div class="field-wrap">
				<core:if test="${precreate=='Yes'}" >
				Course and Payment < <u>Confirmation</u>
				</core:if>
				<core:if test="${precreate=='No'}" >
				Course < Students < Review < Payment < <u>Confirmation</u>
				</core:if>
			</div>
			</core:if>
			<core:if test="${courseRecord.feeOption=='Facility-fee'}" >
            <div class="field-wrap">
                Course < Payment < <u>Confirmation</u>
            </div>
            </core:if>
			<form:form method="POST" commandName="courseRecord" target="_blank">
				<div class="simple-form tabular-form">
					<div class="fieldset-group">
						...
						<core:if test="${courseRecord.action == 'Confirmation'}" >
						<fieldset>
							<legend>
								<!--
								<core:if test="${courseRecord.feeOption=='Fee' && precreate == 'No'}" >
								<p>Your course record has been sent to the Chapter. Once the Chapter reviews and confirms the information, you will be provided with a link to print certificates.</p>
								</core:if>
								-->
								<core:if test="${courseRecord.feeOption=='Fee' && precreate == 'Yes'}" >
								<p>Your class has been scheduled and your payment method has been identified.</p>
								</core:if>
								<core:if test="${courseRecord.feeOption=='Non-fee'}" >
									<core:choose>
										<core:when test="${fn:length(courseRecordList.courseRecordList) > 1}">
											<p>Your Course Records have been successfully submitted.</p>
										</core:when>
										<core:otherwise>
											<p>Your Course Record has been successfully submitted.</p>
										</core:otherwise>
									</core:choose>												
								</core:if>							
								...
								<core:if test="${courseRecord.feeOption=='Facility-fee'}" >
                                    <p>Course Record No: <core:out value="${courseRecord.sheetNumber}" /> submitted.</p>
                                </core:if>
							</legend>
						</fieldset>
						</core:if>
				...
				<core:if test="${courseRecord.action == 'Confirmation' && precreate == 'No'}" >
					<div class="action plain-button alt-button" title="Provides a summary of the course record entered.">
						<div class="bw">
							<input type="button" class="button" onClick="print();" name="" value="Print Summary">	
						</div>
					</div>
					<core:if test="${courseRecord.feeOption=='Fee' && !courseRecord.studentDetails}" >
						<div class="action plain-button alt-button" title="Provides for option to print each student certificate on a full page.">
							<div class="bw">
								<input id="printFullCertificates" type="button" class="button" name="printFullCertificates" value="Print 8.5 x 11 Certificates">	
							</div>
						</div>
						<div class="action plain-button alt-button" title="Provides for option to print wallet-size certificates on standard business card cardstock (e.g. Avery 5371 or 08871, Staples 610837, or Office Depot 388302).">
							<div class="bw">
								<input id="printWalletCertificates" type="button" class="button" name="printWalletCertificates" value="Print Wallet Certificates">	
							</div>
						</div>
					</core:if>
					<div class="action plain-button alt-button" title="Return to the 'Search Existing Course Records' page.">
						<div class="bw">
							<input id="close" type="button" class="button" name="close" value="Close">	
						</div>
					</div>
				</core:if>
				<core:if test="${courseRecord.action == 'Confirmation' && precreate == 'Yes'}" >
